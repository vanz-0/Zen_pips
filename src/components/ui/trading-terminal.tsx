...
function TradingViewIframeChart({ symbol }: { symbol: string }) {
    const { theme } = useTheme();

    return (
        <div className="absolute inset-0 w-full h-full bg-[var(--card-bg)]">
            <iframe
                src={`/chart.html?symbol=${encodeURIComponent(symbol)}&theme=${theme === 'dark' ? 'dark' : 'light'}`}
        ...
        <div className="w-full flex flex-col" style={{ height: "calc(100dvh - 73px)" }}>
            ...
            <div className="flex flex-1 overflow-hidden flex-col lg:flex-row">
                ...
                <div className="flex-1 relative min-w-0 min-h-[300px] lg:min-h-0 bg-[var(--card-bg)] w-full">
                    <TradingViewIframeChart symbol={selectedSymbol} />
                </div>
